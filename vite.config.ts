...
export default defineConfig(({ mode }) => {
  ...
  return {
    // Sub-path deploys (e.g. a GitHub Pages project site at
    // https://<user>.github.io/<repo>/) need every asset URL prefixed with
    // that repo path. GH_PAGES_BASE lets CI pass it in at build time
    // without touching local dev/preview, which still default to '/'.
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/.figma/**'] },
      // Sends /api/* requests to the Express backend (server/, run
      // separately with `npm start` inside that folder) during local dev.
      // In production, /vercel.json routes /api/* to the same backend as
      // a serverless function on the same domain, so no proxy is needed
      // there — this only matters for `npm run dev` on your machine.
      proxy: {
        '/api': {
      ...
      // Figma Make's cloud preview serves this app behind an HTTPS proxy on
      // port 443, so the scaffold hardcodes hmr.clientPort/protocol for that
      // proxy. Outside Figma Make (plain `npm run dev` on localhost) that
      // forces the browser to open a wss:// socket to a port nothing is
      // listening on, which spams the console with connection errors. Only
      // apply the Figma-specific override when that proxy is actually in
      // front of us.
